            ...
            <div class="widget-header">
                ...
                <form action="{{route('balance_p.update',$data->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input required type="text" name="desc_ar" value="{{$data->desc_ar}}" class="form-control"
                               id="desc_ar">
                    </div>
                    <div class="form-group mb-4">
                        <label for="title_ar">{{ __('messages.desc_en') }}</label>
                        <input required type="text" name="desc_en" value="{{$data->desc_en}}" class="form-control"
                               id="desc_en">
                    </div>
                    <div class="form-group mb-4">
                        <label for="title_ar">{{ __('messages.price') }}</label>
                        <input required type="number" name="price" value="{{$data->price}}" step="any"
                               class="form-control" id="price">
                    </div>
                    <div class="form-group mb-4">
                        <label for="title_ar">{{ __('messages.amount') }}</label>
                        <input required type="number" name="amount" value="{{$data->amount}}" step="any"

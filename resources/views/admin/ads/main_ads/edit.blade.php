                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select id="ad_type" name="type" class="form-control">
                            ...
                            <option
                                {{ $data['ad']['type'] == 'link' ? 'selected' : '' }} value="1">{{ __('messages.outside_the_app') }}</option>
                            <option
                                {{ $data['ad']['type'] == 'id' ? 'selected' : '' }} value="2">{{ __('messages.inside_the_app') }}</option>
                        </select>
                    </div>

                    <div style="display: {{ $data['ad']['type'] == 'id' ? 'none' : '' }}"
                         class="form-group mb-4 outside">
                        <label for="link">{{ __('messages.link') }}</label>
                        <input required type="text" name="content" class="form-control" id="link"
                               placeholder="{{ __('messages.link') }}" value="{{ $data['ad']['content'] }}">
                    </div>
                    ...
                    <div style="display: {{ $data['ad']['type'] == 'link' ? 'none' : '' }}" class="form-group inside">
                        ...
                        <select id="users" class="form-control">
                            ...
                            @foreach ($data['users'] as $user)
                                <option
                                    {{ isset($data['product']['user']) && $data['product']['user']['id'] == $user->id ? 'selected' : ''  }} value="{{ $user->id }}">{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>

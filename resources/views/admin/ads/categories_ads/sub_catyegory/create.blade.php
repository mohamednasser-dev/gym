...
@section('content')
    <div class="col-lg-12 col-12 layout-spacing">
        <div class="statbox widget box box-shadow">
            ...
            @if(Route::current()->getName() == 'sub_categories_ads.create_new')
                <form action="{{route('sub_categories_ads.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input required type="hidden" name="id" value="{{$id}}" class="form-control">
                    @else
                        <form action="{{route('sub_categories_ads.store_all_categories',$id)}}" method="post"

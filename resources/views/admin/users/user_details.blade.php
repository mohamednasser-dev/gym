    ...
    <div id="tableSimple" class="col-lg-12 col-12 layout-spacing">
        <div class="statbox widget box box-shadow">
            ...
            <div class="widget-content widget-content-area">
                ...
                <br>
                <form action="/admin-panel/users/send_notifications/{{ $data['user']['id'] }}" method="post"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="form-group mb-4">
                        <label for="image">{{ __('messages.image') }}</label>
                        <input type="file" name="image" class="form-control" id="image"
                               placeholder="{{ __('messages.image') }}" value="">
                    </div>
                    <div class="form-group mb-4">
                        <label for="title">{{ __('messages.notification_title') }}</label>
                        <input required type="text" name="title" class="form-control" id="title"
                               placeholder="{{ __('messages.notification_title') }}" value="">
                    </div>
                    <div class="form-group mb-4">
                        <label for="body">{{ __('messages.notification_body') }}</label>
                        <input required type="text" name="body" class="form-control" id="body"
                               placeholder="{{ __('messages.notification_body') }}" value="">
                    </div>

...
@section('content')
    ...
    <div class="col-lg-12 col-12 layout-spacing">
        <div class="statbox widget box box-shadow">
            ...
            <form method="post" action="{{route('branches.update',$data->id)}}" enctype="multipart/form-data">
                @csrf
                <div class="form-group mb-4">
                    <label for="name">{{ __('messages.branch_ar') }}</label>
                    <input required type="text" name="title_ar" value="{{$data->title_ar}}" class="form-control" id="title_ar">
                </div>
                <div class="form-group mb-4">
                    <label for="email">{{ __('messages.branch_en') }}</label>
                    <input required type="text" class="form-control" value="{{$data->title_en}}" id="title_en" name="title_en">
                </div>

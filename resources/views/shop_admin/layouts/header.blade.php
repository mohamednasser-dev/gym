        ...
        <ul class="navbar-item theme-brand flex-row  text-center">
            <li class="nav-item theme-logo">
                <a href="{{route('hall.home')}}">
                    <img
                        src="{{image_cloudinary_url()}}<?=Auth::guard('shop')->user()->logo ?>"
                        class="navbar-logo" style="height: 50px;" alt="logo">
                </a>
            </li>
            <li class="nav-item theme-text">
                @if(App::isLocale('en'))
                    <a href="{{route('hall.home')}}" class="nav-link"> <?=Auth::guard('shop')->user()->name?></a>
                @else
                    <a href="{{route('hall.home')}}" class="nav-link"> <?=Auth::guard('shop')->user()->name?></a>
                @endif
            </li>
                ...
                <div class="dropdown-menu position-absolute" aria-labelledby="language-dropdown">
                    ...
                    @else
                        <a class="dropdown-item d-flex" href="/setlocale/en"><img src="/admin/assets/img/ca.png" class="flag-width" alt="flag">
                            <span class="align-self-center">&nbsp;{{ __('messages.english') }}</span>
                        </a>
                    @endif
                </div>
            </li>
                ...
                <div class="dropdown-menu position-absolute" aria-labelledby="userProfileDropdown">
                    <div class="">
                        <div class="dropdown-item">
                            <a class="" href="{{route('shop.profile')}}">
...
<div class="sub-header-container">
    <header class="header navbar navbar-expand-sm">
        ...
        @if(Auth::guard('shop')->user()->famous == '1')
            <ul class="navbar-nav flex-row ml-auto " style="padding-top: 5px;">
                <span class="btn btn-warning mb-2 mr-2 rounded-circle">
                    <i class="far fa-star"></i>
                </span>
                <span style="padding-top: 12px;padding-left: 20px;">{{ __('messages.choosen_shop_famous') }}</span>
            </ul>
        @endif
    </header>
</div>

        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-header">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input required type="text" name="title_ar" class="form-control" id="title_ar"
                               placeholder="{{ __('messages.title_ar') }}" value="{{ $data['option']['title_ar'] }}">
                    </div>
                    <div class="form-group mb-4">
                        <label for="title_en">{{ __('messages.title_en') }}</label>
                        <input required type="text" name="title_en" class="form-control" id="title_en"
                               placeholder="{{ __('messages.title_en') }}" value="{{ $data['option']['title_en'] }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select id="category_select" name="category_ids[]" class="form-control tags"
                            ...
                            @foreach ( $data['categories'] as $category )
                                <option
                                    {{ in_array($category->id, $data['categories_array']) ? 'selected' : '' }} value="{{ $category->id }}">{{ App::isLocale('en') ? $category->title_en : $category->title_ar }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-4">
                ...
                @if(count($data['option']->values) > 0)
                        ...
                        {{ $n < count($data['option']->values) - 1 ? $value->value_en . "," : $value->value_en }}
                        @php
                            $n ++;
                        @endphp
                        @endforeach
                    ...
                    <div class="form-group mb-4">
                        <label for="value_ar">{{ __('messages.value_ar') }} ( {{ __('messages.values_number_should') }}
                            )</label><br/>
                ...
                @if(count($data['option']->values) > 0)
                        ...
                        @endphp
                        @foreach($data['option']->values as $value)
                        {{ $i < count($data['option']->values) - 1 ? $value->value_ar . "," : $value->value_ar }}
                        @php
                            $i ++;
                        @endphp
                        @endforeach
